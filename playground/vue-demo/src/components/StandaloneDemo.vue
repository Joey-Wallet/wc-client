<template>
  <div class="w-full">
    <div class="bg-color1 rounded-lg p-6 border border-color6 mb-4 shadow-sm w-full max-w-md transition-all duration-300">
      <h2 class="text-xl font-semibold mb-4 text-color12">Standalone Provider Demo</h2>
      <p class="text-color11 mb-4">Using the standalone provider with direct Universal Provider integration.</p>
      ...
      <div v-if="chains && chains.length" class="mb-4">
        <label for="chain-select-standalone" class="text-color12 font-medium mb-2 block">Switch Chain:</label>
        <select 
          id="chain-select-standalone" 
          :value="chain" 
      ...
      <div v-if="provider" class="mt-8">
        <h3 class="text-lg font-semibold text-color12 mb-2">Provider Status:</h3>
        <div class="bg-color3 border border-color6 rounded-md p-4 mb-4 border-l-4 border-l-blue9">
          <div class="text-color11 mb-1"><strong class="text-color12">Connected:</strong> {{ !!session }}</div>
          <div class="text-color11 mb-1 break-words overflow-hidden"><strong class="text-color12">Provider URI:</strong> <span class="break-all">{{ provider.uri || 'None' }}</span></div>
          <div class="text-color11 break-words overflow-hidden"><strong class="text-color12">Session Topic:</strong> <span class="break-all">{{ session?.topic || 'None' }}</span></div>
        </div>
      </div>
